...
export async function crearCotizacion(cliente, items, notas = '', venceDias = 7) {
  ...
  const { data: cot, error: errCot } = await supabase
    .from('cotizaciones')
    .insert({ cliente: cliente || 'Sin nombre', total, notas, vence_en: vence.toISOString(), tienda_id: u.tienda_id })
    .select().single()
  if (errCot) throw errCot
  const detalle = items.map(i => ({
    cotizacion_id: cot.id, producto_id: i.producto_id,
    nombre_producto: i.nombre, cantidad: i.cantidad,
    precio_unitario: i.precio_unitario, subtotal: i.precio_unitario * i.cantidad
  }))
...
export async function getCotizaciones() {
  const { data, error } = await supabase
    .from('cotizaciones').select('*, detalle_cotizaciones(*)')
